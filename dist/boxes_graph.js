...
async function add_edge(edgeData, callback) {
    ...
    const labels = box_ids.map((id) => [id, 0]);
    const has_cycle = detect_cycle(build_graph(box_ids, this.weights, staged), new Map(labels), v);
    if (has_cycle) {
        callback(null);
...
export async function refresh_boxes_graph() {
    ...
    const weight_by_title = new Map(box_weights.map(entry => [entry.box, entry.total_weight]));
    const weights = new Map(boxes.map(box => [box.id, weight_by_title.get(box.title) ?? 0]));
...
export function init_boxes_graph() {
...
//# sourceMappingURL=boxes_graph.js.map