...
function add_log_entry(message: string, is_error: boolean = false) {
    const log_element = get_element<HTMLDivElement>("intakeLog");
...
function update_category_datalist(filter_value: string = "") {
    const datalist = get_element<HTMLDataListElement>("categoryDatalist");
    datalist.innerHTML = "";

    const filtered = categories.filter(cat =>
        cat.toLowerCase().includes(filter_value.toLowerCase())
    );
...
async function load_items(category: string) {
...
async function add_item() {
    const manager = get_db_manager();
    const categoryInput = get_element<HTMLInputElement>("categoryInput");
    const expiryDateInput = get_element<HTMLInputElement>("expiryDate");
    const boxNumberInput = get_element<HTMLInputElement>("boxNumber");
    const statusSelect = get_element<HTMLSelectElement>("status");
    ...
    try {
        if (!category || expiry_date === null || isNaN(box_number)) throw new Error("Invalid input values");
...
async function add_category() {
    const manager = get_db_manager();
    const newCategoryInput = get_element<HTMLInputElement>("newCategory");
    const categoryInput = get_element<HTMLInputElement>("categoryInput");
    const category = newCategoryInput.value.trim();

    try {
        if (!category) throw new Error("Category cannot be empty");
        if (categories.includes(category)) throw new Error("Category already exists");
...
function setup_event_listeners() {
    const intakeForm = get_element<HTMLFormElement>("intakeForm");
    intakeForm.addEventListener("submit", async (e) => {
        e.preventDefault();
        await add_item();
    });

    const categoryForm = get_element<HTMLFormElement>("categoryForm");
